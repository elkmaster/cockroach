...
import { Button, DatePicker, TimePicker, notification, Calendar, Icon } from "antd";
...
class RangeSelect extends React.Component<RangeSelectProps, RangeSelectState> {
  ...
  renderDatePickerAddon = (direction: DateTypes) => <div className="calendar-today-btn"><Button onClick={() => this.onChangeDate(direction)(moment())} type="link" size="small">Today</Button></div>;
  ...
  headerRender = (item: any) => (
    <div className="calendar-month-picker">
      <Button type="link" onClick={() => item.onChange(moment(item.value).subtract(1, "months"))}><Icon type="left" /></Button>
      <span>{moment(item.value).format("MMM YYYY")}</span>
      <Button type="link" onClick={() => item.onChange(moment(item.value).add(1, "months"))}><Icon type="right" /></Button>
    </div>
  )
